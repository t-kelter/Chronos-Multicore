/*
 * modified bpred.c - branch predictor routines for SS_SMT 1.0 simulator
 *
 * This file was adapted by Ronaldo A. L. Goncalves using 
 * the original bpred.c file from SimpleScalar 3.0 Tool.
 *
 ...
 *
 * The original bpred.c file is a part of the SimpleScalar tool suite written by
 ...
 * The original bpred.c was adapted to support the ss_smt.c version 1.0.
 ...
 *
 *
 * $Id: bpred.c,v 1.4 1998/08/27 07:56:23 taustin Exp $
 *
 * $Log: bpred.c,v $
             ...
             int sn)                     /* slot number */
{
  ...
  case BPredComb:
    /* bimodal component */
    pred->dirpred.bimod = 
      bpred_dir_create(BPred2bit, bimod_size, 0, 0, 0,sn);

    /* 2-level component */
    pred->dirpred.twolev = 
      bpred_dir_create(BPred2Level, l1size, l2size, shift_width, xor,sn);

    /* metapredictor component */
    pred->dirpred.meta = 
      bpred_dir_create(BPred2bit, meta_size, 0, 0, 0,sn);

    break;

  case BPred2Level:
    pred->dirpred.twolev = 
      bpred_dir_create(class, l1size, l2size, shift_width, xor,sn);

    break;

  case BPred2bit:
    pred->dirpred.bimod = 
      bpred_dir_create(class, bimod_size, 0, 0, 0,sn);
  ...
  default:
    panic(sn,"bogus predictor class");
  }
  ...
  case BPred2bit:
	...
	fatal(sn,"number of BTB sets must be non-zero and a power of two");
      if (!btb_assoc || (btb_assoc & (btb_assoc-1)) != 0)
	fatal(sn,"BTB associativity must be non-zero and a power of two");

      if (!(pred->btb.btb_data = calloc(btb_sets * btb_assoc,
					sizeof(struct bpred_btb_ent_t))))
	fatal(sn,"cannot allocate BTB");
	...
	fatal(sn,"Return-address-stack size must be zero or a power of two");
      
      pred->retstack.size = retstack_size;
      if (retstack_size)
	if (!(pred->retstack.stack = calloc(retstack_size, 
					    sizeof(struct bpred_btb_ent_t))))
	  fatal(sn,"cannot allocate return-address-stack");
  ...
  int sn)                       /* slot number */
{
  ...
  case BPred2Level:
	...
	fatal(sn,"level-1 size, `%d', must be non-zero and a power of two", 
	      l1size);
      pred_dir->config.two.l1size = l1size;
      
      if (!l2size || (l2size & (l2size-1)) != 0)
	fatal(sn,"level-2 size, `%d', must be non-zero and a power of two", 
	      l2size);
      pred_dir->config.two.l2size = l2size;
      
      if (!shift_width || shift_width > 30)
	fatal(sn,"shift register width, `%d', must be non-zero and positive",
	      shift_width);
  ...
  case BPred2bit:
    if (!l1size || (l1size & (l1size-1)) != 0)
      fatal(sn,"2bit table size, `%d', must be non-zero and a power of two", 
	    l1size);
    pred_dir->config.bimod.size = l1size;
    if (!(pred_dir->config.bimod.table =
	  calloc(l1size, sizeof(unsigned char))))
      fatal(sn,"cannot allocate 2bit storage");
    /* initialize counters to weakly this-or-that */
    flipflop = 1;
  ...
  default:
    panic(sn,"bogus branch direction predictor class");
  }
             ...
             int sn)                    /* slot number */
{
  switch (pred->class) {
  case BPredComb:
    bpred_dir_config (pred->dirpred.bimod, "bimod", stream,sn);
    bpred_dir_config (pred->dirpred.twolev, "2lev", stream,sn);
    bpred_dir_config (pred->dirpred.meta, "meta", stream,sn);
  ...
  case BPredTaken:
    bpred_dir_config (pred->dirpred.bimod, "taken", stream,sn);
    break;
  case BPredNotTaken:
    bpred_dir_config (pred->dirpred.bimod, "nottaken", stream,sn);
    break;

  default:
    panic(sn,"bogus branch predictor class");
  }
}
...
{
  char buf[128], buf1[128], bpred_name[MAX_SLOTS][128];
  int sn;

  for (sn=0;sn<process_num;sn++)
     /* get a name for this predictor */
     switch (pred[sn]->class) {
        case BPredComb:
           sprintf(bpred_name[sn], "bpred_comb");
           break;
        case BPred2Level:
           sprintf(bpred_name[sn], "bpred_2lev");
           break;
        case BPred2bit:
           sprintf(bpred_name[sn], "bpred_bimod");
           break;
        case BPredTaken:
           sprintf(bpred_name[sn], "bpred_taken");
           break;
        case BPredNotTaken:
           sprintf(bpred_name[sn], "bpred_nottaken");
           break;
        default:
           panic(sn,"bogus branch predictor class");
    }
  ...
  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.lookups_%s", bpred_name[sn], itoa[sn]);
     stat_reg_counter(sdb, buf, "total number of bpred lookups",
		   &pred[sn]->lookups, 0, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.updates_%s", bpred_name[sn], itoa[sn]);
     sprintf(buf1, "%s.dir_hits_%s + %s.misses_%s", bpred_name[sn], itoa[sn], bpred_name[sn], itoa[sn]);
     stat_reg_formula(sdb, buf, "total number of updates", buf1, "%12.0f",sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.addr_hits_%s", bpred_name[sn], itoa[sn]);
     stat_reg_counter(sdb, buf, "total number of address-predicted hits", 
		   &pred[sn]->addr_hits, 0, NULL,sn);
  } 

  for (sn=0;sn<process_num;sn++) {
    sprintf(buf, "%s.dir_hits_%s", bpred_name[sn], itoa[sn]);
    stat_reg_counter(sdb, buf, 
		   "total number of direction-predicted hits "
		   "(includes addr-hits)", 
		   &pred[sn]->dir_hits, 0, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) 
     if (pred[sn]->class == BPredComb)
     {
         sprintf(buf, "%s.used_bimod_%s", bpred_name[sn], itoa[sn]);
         stat_reg_counter(sdb, buf, 
		       "total number of bimodal predictions used", 
		       &pred[sn]->used_bimod, 0, NULL,sn);

         sprintf(buf, "%s.used_2lev_%s", bpred_name[sn], itoa[sn]);
         stat_reg_counter(sdb, buf, 
		       "total number of 2-level predictions used", 
		       &pred[sn]->used_2lev, 0, NULL,sn);
     }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.misses_%s", bpred_name[sn], itoa[sn]);
     stat_reg_counter(sdb, buf, "total number of misses", &pred[sn]->misses, 0, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.jr_hits_%s", bpred_name[sn], itoa[sn]);
     stat_reg_counter(sdb, buf,
		   "total number of address-predicted hits for JR's",
		   &pred[sn]->jr_hits, 0, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.jr_seen_%s", bpred_name[sn], itoa[sn]);
     stat_reg_counter(sdb, buf,
		   "total number of JR's seen",
		   &pred[sn]->jr_seen, 0, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.jr_non_ras_hits.PP_%s", bpred_name[sn], itoa[sn]);
     stat_reg_counter(sdb, buf,
		   "total number of address-predicted hits for non-RAS JR's",
		   &pred[sn]->jr_non_ras_hits, 0, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.jr_non_ras_seen.PP_%s", bpred_name[sn], itoa[sn]);
     stat_reg_counter(sdb, buf,
		   "total number of non-RAS JR's seen",
		   &pred[sn]->jr_non_ras_seen, 0, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.bpred_addr_rate_%s", bpred_name[sn], itoa[sn]);
     sprintf(buf1, "%s.addr_hits_%s / %s.updates_%s", bpred_name[sn], itoa[sn], bpred_name[sn], itoa[sn]);
  ...
  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.bpred_dir_rate_%s", bpred_name[sn], itoa[sn]);
     sprintf(buf1, "%s.dir_hits_%s / %s.updates_%s", bpred_name[sn], itoa[sn], bpred_name[sn], itoa[sn]);
     stat_reg_formula(sdb, buf,
  ...
  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.bpred_jr_rate_%s", bpred_name[sn], itoa[sn]);
     sprintf(buf1, "%s.jr_hits_%s / %s.jr_seen_%s", bpred_name[sn], itoa[sn], bpred_name[sn], itoa[sn]);
     stat_reg_formula(sdb, buf,
  ...
  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.bpred_jr_non_ras_rate.PP_%s", bpred_name[sn], itoa[sn]);
     sprintf(buf1, "%s.jr_non_ras_hits.PP_%s / %s.jr_non_ras_seen.PP_%s", bpred_name[sn], itoa[sn], bpred_name[sn], itoa[sn]);
     stat_reg_formula(sdb, buf,
  ...
  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.retstack_pushes_%s", bpred_name[sn], itoa[sn]);
     stat_reg_counter(sdb, buf,
		   "total number of address pushed onto ret-addr stack",
		   &pred[sn]->retstack_pushes, 0, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.retstack_pops_%s", bpred_name[sn], itoa[sn]);
     stat_reg_counter(sdb, buf,
		   "total number of address popped off of ret-addr stack",
		   &pred[sn]->retstack_pops, 0, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.used_ras.PP_%s", bpred_name[sn], itoa[sn]);
     stat_reg_counter(sdb, buf,
		   "total number of RAS predictions used",
		   &pred[sn]->used_ras, 0, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.ras_hits.PP_%s", bpred_name[sn], itoa[sn]);
     stat_reg_counter(sdb, buf,
		   "total number of RAS hits",
		   &pred[sn]->ras_hits, 0, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.ras_rate.PP_%s", bpred_name[sn], itoa[sn]);
     sprintf(buf1, "%s.ras_hits.PP_%s / %s.used_ras.PP_%s", bpred_name[sn], itoa[sn], bpred_name[sn], itoa[sn]);
     stat_reg_formula(sdb, buf,
             ...
             int sn)                    /* slot number */
{
  ...
  if (!dir_update_ptr)
    panic(sn,"no bpred update record");
  ...
  switch (pred->class) {
      ...
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	  ...
	  bimod = bpred_dir_lookup (pred->dirpred.bimod, baddr, sn);
	  twolev = bpred_dir_lookup (pred->dirpred.twolev, baddr,sn);
	  meta = bpred_dir_lookup (pred->dirpred.meta, baddr,sn);
      ...
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	  dir_update_ptr->pdir1 =
	    bpred_dir_lookup (pred->dirpred.twolev, baddr, sn);
	}
      break;
    case BPred2bit:
      if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND))
	{
	  dir_update_ptr->pdir1 =
	    bpred_dir_lookup (pred->dirpred.bimod, baddr, sn);
	}
      break;
    case BPredTaken:
      return btarget;
