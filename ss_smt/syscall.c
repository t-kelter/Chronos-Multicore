/*
 * modified syscall.c - proxy system call handler routines for SS_SMT 1.0 simulator
 *
 * This file was adapted by Ronaldo A. L. Goncalves using 
 * the original syscall.c file from SimpleScalar 3.0 Tool.
 *
 ...
 *
 * The original syscall.c file is a part of the SimpleScalar tool suite written by
 ...
 * The original syscall.c was adapted to support the ss_smt.c version 1.0.
 * The main adaptations were:
 * 1) The function syscall was modified to include the "slot's number" parameter passing. 
 * 2) The function syscall was adapted in the "case" statement on the option 
 *    "SS_SYS_exit" in order to don't abort the processing before all slot have
 *    executed the current cycle. Otherwise, both finished and exit_slot are 
 *    set to allow this control.
 *
 * $Id: syscall.c,v 1.7 1998/08/31 17:13:29 taustin Exp taustin $
 *
 * $Log: syscall.c,v $
            ...
            int sn)                     /* slot number */
{
  ...
  if (traceable && sim_eio_fd[sn] != NULL)
    {
      eio_read_trace(sim_eio_fd[sn], sim_num_insn[sn], regs, mem_fn, mem, inst,sn);
  ...
  switch (syscode)
    {
    case SS_SYS_exit:
      finished = TRUE; /* set execution to end after the processing */
	...
	fatal(sn,"vfork() in SYS_vfork failed");
      if (regs->regs_R[2] != 0)
	{
	  regs->regs_R[3] = 0;
	  regs->regs_R[7] = 0;
	...
	if (!(buf = (char *)calloc(/*nbytes*/regs->regs_R[6], sizeof(char))))
	  fatal(sn,"out of memory in SYS_read");

        if (regs->regs_R[4] == 0) /* read from stdin */
        {   if (sim_inputfd[sn] != NULL) /* stdin must be redirected */
            {  /* read data from input file */
                /*nread*/regs->regs_R[2] =
                 fread(buf, 1, /*nbytes*/regs->regs_R[6], sim_inputfd[sn]);
            }
	...
	if (!(buf = (char *)calloc(/*nbytes*/regs->regs_R[6], sizeof(char))))
	  fatal(sn,"out of memory in SYS_write");

	/* copy inputs into host memory */
	mem_bcopy(mem_fn, mem,
		  Read, /*buf*/regs->regs_R[5],
		  buf, /*nbytes*/regs->regs_R[6],sn);
	...
	if (ss_flags != 0)
	  fatal(sn,"syscall: open: cannot decode flags: 0x%08x", ss_flags);

	/* copy filename to host memory */
	mem_strcpy(mem_fn, mem, Read, /*fname*/regs->regs_R[4], buf,sn);
    ...
    case SS_SYS_chown:
#ifdef _MSC_VER
      warn(sn,"syscall chown() not yet implemented for MSC...");
      regs->regs_R[7] = 0;
#else /* !_MSC_VER */
      {
	char buf[MAXBUFSIZE];
	...
	if (addr >= ld_brk_point[sn] && addr < (md_addr_t)regs->regs_R[29])
	  {
	    regs->regs_R[2] = 0;
	    regs->regs_R[7] = 0;
	    ld_brk_point[sn] = addr;
	  }
	else
	  {
    ...
    case SS_SYS_getuid:
#ifdef _MSC_VER
      warn(sn,"syscall getuid() not yet implemented for MSC...");
      regs->regs_R[7] = 0;
#else /* !_MSC_VER */
      /* get current user id */
	...
	if (syscode == SS_SYS_stat)
	  ...
	  {
#ifdef _MSC_VER
	    warn(sn,"syscall lstat() not yet implemented for MSC...");
	    regs->regs_R[7] = 0;
	    break;
#else /* !_MSC_VER */
	...
	mem_bcopy(mem_fn, mem, Write, /*sbuf*/regs->regs_R[5],
		  &ss_sbuf, sizeof(struct ss_statbuf),sn);
      }
      break;
	...
	mem_bcopy(mem_fn, mem, Read, /*fd's*/regs->regs_R[4], fd, sizeof(fd),sn);
    ...
    case SS_SYS_getgid:
#ifdef _MSC_VER
      warn(sn,"syscall getgid() not yet implemented for MSC...");
      regs->regs_R[7] = 0;
#else /* !_MSC_VER */
	...
	if (!local_req)
	  {
	    ...
	    warn(sn,"syscall: ioctl: ioctl code not supported d=%d, req=%d",
	...
	else
	  {
	    ...
	    if (/*argp*/regs->regs_R[6] != 0)
	      mem_bcopy(mem_fn, mem,
			Read, /*argp*/regs->regs_R[6], buf, NUM_IOCTL_BYTES,sn);
    ...
    case SS_SYS_setitimer:
      ...
      warn(sn,"syscall: setitimer ignored");
      break;
	...
	warn(sn,"syscall: called getdtablesize()\n");
	regs->regs_R[2] = 16;
	regs->regs_R[7] = 0;
      }
#else
    ...
    case SS_SYS_fcntl:
#ifdef _MSC_VER
      warn(sn,"syscall fcntl() not yet implemented for MSC...");
      regs->regs_R[7] = 0;
#else /* !_MSC_VER */
    ...
    case SS_SYS_select:
#ifdef _MSC_VER
      warn(sn,"syscall select() not yet implemented for MSC...");
      regs->regs_R[7] = 0;
#else /* !_MSC_VER */
      {
	...
	if (/*timeout*/param5 != 0)
	  {
	    mem_bcopy(mem_fn, mem, Read, /*timeout*/param5,
		      &timeout, sizeof(struct timeval),sn);
	    timeoutp = &timeout;
	  }
	else
	  timeoutp = NULL;
...
#ifdef _MSC_VER
      warn(sn,"syscall gettimeofday() not yet implemented for MSC...");
      regs->regs_R[7] = 0;
#else /* _MSC_VER */
      {
	...
	if (/*timeval*/regs->regs_R[4] != 0)
	  {
	    ...
	    mem_bcopy(mem_fn, mem, Read, /*timeval*/regs->regs_R[4],
		      &ss_tv, sizeof(struct ss_timeval),sn);
	...
	if (/*timezone*/regs->regs_R[5] != 0)
	  {
	    ...
	    mem_bcopy(mem_fn, mem, Read, /*timezone*/regs->regs_R[5],
		      &ss_tz, sizeof(struct ss_timezone),sn);
	...
	mem_bcopy(mem_fn, mem, Write, /*rusage*/regs->regs_R[5],
		  &rusage, sizeof(struct ss_rusage),sn);
      }
	...
	mem_bcopy(mem_fn, mem, Write, /*rusage*/regs->regs_R[5],
	    ...
	    warn(sn,"syscall: called getrusage()\n");
            regs->regs_R[7] = 0;
#else
#error No getrusage() implementation!
    ...
    case SS_SYS_writev:
#ifdef _MSC_VER
      warn(sn,"syscall writev() not yet implemented for MSC...");
      regs->regs_R[7] = 0;
#else /* !_MSC_VER */
      {
	int i;
...
#error No utimes() implementation!
	...
	else
	  {
	    ...
	    mem_bcopy(mem_fn, mem, Read, /*timeout*/regs->regs_R[5],
		      ss_tval, 2*sizeof(struct ss_timeval),sn);
...
#error No utimes() implementation!
...
#ifdef _MSC_VER
      warn(sn,"syscall get/setrlimit() not yet implemented for MSC...");
      regs->regs_R[7] = 0;
#elif defined(__CYGWIN32__)
      warn(sn,"syscall: called get/setrlimit()\n");
      regs->regs_R[7] = 0;
#else
      {
	...
	mem_bcopy(mem_fn, mem, Read, /*rlimit*/regs->regs_R[5],
		  &ss_rl, sizeof(struct ss_rlimit),sn);
	...
	if (!buf)
	  fatal(sn,"out of memory in SYS_getdirentries");
    ...
    default:
      panic(sn,"invalid/unimplemented system call encountered, code %d", syscode);
    }
}
