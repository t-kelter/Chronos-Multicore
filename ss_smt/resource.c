/*
 * modified resource.c -  resource manager routines for SS_SMT 1.0 simulator
 *
 * This file was adapted by Ronaldo A. L. Goncalves using 
 * the original resource.c file from SimpleScalar 3.0 Tool.
 *
 ...
 *
 * The original resource.c file is a part of the SimpleScalar tool suite written by
 ...
 * The original resource.c was adapted to support the ss_smt.c version 1.0.
 ...
 *  
 * $Id: resource.c,v 1.4 1998/08/27 15:54:06 taustin Exp taustin $
 *
 * $Log: resource.c,v $
...
struct res_pool *
res_create_pool(char *name, struct res_desc *pool, int ndesc, int sn)
{
  ...
  for (ninsts=0,i=0; i<ndesc; i++)
    {
      if (pool[i].quantity > MAX_INSTS_PER_CLASS)
        fatal(sn,"too many functional units, increase MAX_INSTS_PER_CLASS");
  ...
  for (index=0,i=0; i<ndesc; i++)
      ...
      for (j=0; j<pool[i].quantity; j++)
	{
	  ...
	  inst_pool[index].quantity = 1; /* update the quantity */
  ...
  if ((res->name = mystrdup(name))==NULL)
