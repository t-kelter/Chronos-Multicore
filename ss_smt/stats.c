/*
 * modified stats.c - statistical package routines for SS_SMT 1.0 simulator
 *
 * This file was adapted by Ronaldo A. L. Goncalves using 
 * the original stats.c file from SimpleScalar 3.0 Tool.
 *
 ...
 * The original stats.c file is a part of the SimpleScalar tool suite written by
 ...
 * The original stats.c was adapted to support the ss_smt.c version 1.0.
 ...
 *   
 * $Id: stats.c,v 1.2 1998/08/27 16:39:40 taustin Exp taustin $
 *
 * $Log: stats.c,v $
                ...
                int sn)  /* slot number */
{
  ...
  switch (stat->sc)
    ...
    case sc_dist:
    case sc_sdist:
      fatal(sn,"stat distributions not allowed in formula expressions");
      break;
    case sc_formula:
      {
...
stat_new(int sn)
{
  ...
  sdb->stats = NULL;
  sdb->evaluator = eval_new(stat_eval_ident, sdb,sn);
             ...
             int sn)
{
 ...
 /* copy the stat name in a new memory space and point it with stat->name */

  if ((stat->name = mystrdup(name))==NULL) 
                 ...
                 int sn)                   /* slot number */
{
  switch (stat->sc)
	...
	if (hash < 0 || hash >= HTAB_SZ)
	  panic(sn,"hash table index overflow");
	...
	bucket->count += nsamples;
      ...
      break;
    default:
      panic(sn,"stat variable is not an array distribution");
    }
}
...
stat_add_sample(struct stat_stat_t *stat,/* stat variable */
		md_addr_t index,	/* index of sample */
               int sn)                   /* slot number */
{
  stat_add_samples(stat, index, 1,sn);
}
                 ...
                 int sn)                   /* slot number */
{
  ...
  if ((stat->variant.for_formula.formula = mystrdup(formula))==NULL)
      fatal(sn,"out of memory in the allocation for %s\n",formula);
	  ...
	  if (stat->variant.for_dist.print_fn)
	      ...
	      else
		{
		  if (pf == (PF_COUNT|PF_PDF|PF_CDF))
		    ...
		    fatal(sn,"distribution format not yet implemented");
		}
	      fprintf(fd, "\n");
	    }
	}
            ...
            int sn)                     /* slot number */
{
  ...
  if (bcount > 0)
    {
      myfprintf(sn,fd, "%s.imin = 0x%p\n", stat->name, imin);
      myfprintf(sn,fd, "%s.imax = 0x%p\n", stat->name, imax);
    }
  else
    {
      fprintf(fd, "%s.imin = %d\n", stat->name, -1);
      ...
      for (i=0; i<bcount; i++)
	{
	  ...
	  if (stat->variant.for_sdist.print_fn)
	  ...
	  else
	    {
	      if (stat->format == NULL)
		{
		  myfprintf(sn,fd, "0x%p ", barr[i]->index);
		  if (pf & PF_COUNT)
                ...
                int sn)                 /* slot number */
{
  ...
  switch (stat->sc)
    ...
    case sc_double:
      ...
      break;
    case sc_dist:
      print_dist(stat, fd,sn);
      break;
    case sc_sdist:
      print_sdist(stat, fd,sn);
      break;
    case sc_formula:
      {
	/* instantiate a new evaluator to avoid recursion problems */
	...
	if (eval_error[sn] != ERR_NOERR || *endp != '\0')
	  ...
	  myfprintf(sn,fd, stat->format, eval_as_double(val,sn));
	fprintf(fd, " # %s", stat->desc);
		 ...
		 FILE *fd)		/* output stream */
{
  ...
  /* all statistics must be replicated by process_num */
  stat=sdb->stats;
  while (stat!=NULL) {

     /* get the slot number */

     sn = get_slot_number(stat->name,&okay);


     stat_print_stat(sdb, stat, fd, sn);
     stat=stat->next;
  }
}
...
main(void)
{
  ...
  stat_add_sample(stat, 8,sn);
  stat_add_sample(stat, 8,sn);
  stat_add_sample(stat, 1,sn);
  stat_add_sample(stat, 3,sn);
  stat_add_sample(stat, 4,sn);
  stat_add_sample(stat, 4,sn);
  stat_add_sample(stat, 7,sn);

  stat_add_sample(stat1, 32,sn);
  stat_add_sample(stat1, 32,sn);
  stat_add_sample(stat1, 1,sn);
  stat_add_sample(stat1, 12,sn);
  stat_add_sample(stat1, 17,sn);
  stat_add_sample(stat1, 18,sn);
  stat_add_sample(stat1, 30,sn);

  stat_add_sample(stat2, 8,sn);
  stat_add_sample(stat2, 8,sn);
  stat_add_sample(stat2, 1,sn);
  stat_add_sample(stat2, 3,sn);
  stat_add_sample(stat2, 4,sn);
  stat_add_sample(stat2, 4,sn);
  stat_add_sample(stat2, 7,sn);

  stat_add_sample(stat3, 8,sn);
  stat_add_sample(stat3, 8,sn);
  stat_add_sample(stat3, 1,sn);
  stat_add_sample(stat3, 3,sn);
  stat_add_sample(stat3, 4,sn);
  stat_add_sample(stat3, 4,sn);
  stat_add_sample(stat3, 7,sn);

  stat_add_sample(stat4, 800,sn);
  stat_add_sample(stat4, 800,sn);
  stat_add_sample(stat4, 1123,sn);
  stat_add_sample(stat4, 3332,sn);
  stat_add_sample(stat4, 4000,sn);
  stat_add_samples(stat4, 4001, 18,sn);
  stat_add_sample(stat4, 7,sn);

  stat_add_sample(stat5, 800,sn);
  stat_add_sample(stat5, 800,sn);
  stat_add_sample(stat5, 1123,sn);
  stat_add_sample(stat5, 3332,sn);
  stat_add_sample(stat5, 4000,sn);
  stat_add_samples(stat5, 4001, 18,sn);
  stat_add_sample(stat5, 7,sn);
