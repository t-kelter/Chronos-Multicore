/*
 * modified expr.h -expression evaluator routines for SS_SMT 1.0 simulator
 *
 * This file was adapted by Ronaldo A. L. Goncalves using 
 * the original expr.h file from SimpleScalar 3.0 Tool.
 *
 ...
 *
 * The original expr.h file is a part of the SimpleScalar tool suite written by
 ...
 * The original expr.h was adapted to support the ss_smt.c version 1.0.
 ...
 *
 * $Id: eval.h,v 1.2 1998/08/27 08:27:01 taustin Exp $
 *
 * $Log: eval.h,v $
...
(*eval_ident_t)(struct eval_state_t *es, int sn); /* ident string in es->tok_buf */
...
/* expression evaluation error, this must be a global. See eval_init for inicialization */
extern enum eval_err_t eval_error[MAX_SLOTS] /* = ERR_NOERR */;
...
double eval_as_double(struct eval_value_t val, int sn);

/* eval_value_t (any numeric type) -> float */
float eval_as_float(struct eval_value_t val, int sn);

#ifdef HOST_HAS_QUAD
/* eval_value_t (any numeric type) -> quad_T */
quad_T eval_as_quad(struct eval_value_t val, int sn);

/* eval_value_t (any numeric type) -> squad_T */
squad_T eval_as_squad(struct eval_value_t val, int sn);
#endif /* HOST_HAS_QUAD */

/* eval_value_t (any numeric type) -> md_addr_t */
md_addr_t eval_as_addr(struct eval_value_t val, int sn);

/* eval_value_t (any numeric type) -> unsigned int */
unsigned int eval_as_uint(struct eval_value_t val, int sn);

/* eval_value_t (any numeric type) -> int */
int eval_as_int(struct eval_value_t val, int sn);
