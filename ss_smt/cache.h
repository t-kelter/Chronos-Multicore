/*
 * modified cache.h - cache module routines for SS_SMT 1.0 simulator
 *
 * This file was adapted by Ronaldo A. L. Goncalves using 
 * the original cache.h file from SimpleScalar 3.0 Tool.
 *
 ...
 *
 * The original cache.h file is a part of the SimpleScalar tool suite written by
 ...
 * The original cache.h was adapted to support the ss_smt.c version 1.0.
 ...
 * user instantiates caches using cache_create().  When instantiated, the user
...
struct cache_t
{
  ...
  int last_sn;                 /* slot's number of last line acessed */
...
/* register stats for multiple cache */
void
...
/* cache access functions, these are safe, they check alignment and
   permissions */
#define cache_double(cp, cmd, addr, p, now, udata, sn)	\
  cache_access(cp, cmd, addr, p, sizeof(double), now, udata, sn)
#define cache_float(cp, cmd, addr, p, now, udata, sn)	\
  cache_access(cp, cmd, addr, p, sizeof(float), now, udata, sn)
#define cache_dword(cp, cmd, addr, p, now, udata, sn)	\
  cache_access(cp, cmd, addr, p, sizeof(long long), now, udata, sn)
#define cache_word(cp, cmd, addr, p, now, udata, sn)	\
  cache_access(cp, cmd, addr, p, sizeof(int), now, udata,sn)
#define cache_half(cp, cmd, addr, p, now, udata, sn)	\
  cache_access(cp, cmd, addr, p, sizeof(short), now, udata, sn)
#define cache_byte(cp, cmd, addr, p, now, udata, sn)	\
  cache_access(cp, cmd, addr, p, sizeof(char), now, udata, sn)
