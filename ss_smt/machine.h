/*
 * modified machine.h - SimpleScalar portable ISA (pisa) definition routines
 *                      for SS_SMT 1.0 simulator
 *
 * This file was adapted by Ronaldo A. L. Goncalves using 
 * the original machine.h file from SimpleScalar 3.0 Tool.
 *
 ...
 *
 * The original machine.h file is a part of the SimpleScalar tool suite written by
 ...
 * The original machine.h was adapted to support the ss_smt.c version 1.0.
 ...
 *
 * $Id: pisa.h,v 1.7 1998/08/31 17:13:14 taustin Exp taustin $
 *
 * $Log: pisa.h,v $
...
#define MD_TEXT_BASE		0x00400000 /* default  0x00400000 */
#define MD_DATA_BASE		0x10000000
#define MD_STACK_BASE 		0x7fffc000

/* maximum size of argc+argv+envp environment = 16k */
...
#define MD_AMODE_PROBE(AM, OP, FSM)					\
  {									\
	...
	if ((BS) == (FSM))						\
      ...
      panic(sn,"cannot decode addressing mode");				\
  }
...
#define MD_MISC_REGS_TO_EXO(REGS)					\
  exo_new(sn,ec_list,							\
	  /*icnt*/exo_new(sn,ec_integer, (exo_integer_t)sim_num_insn[sn]),	\
	  /*PC*/exo_new(sn,ec_address, (exo_integer_t)(REGS)->regs_PC),	\
	  /*NPC*/exo_new(sn,ec_address, (exo_integer_t)(REGS)->regs_NPC),	\
	  /*HI*/exo_new(sn,ec_integer, (exo_integer_t)(REGS)->regs_C.hi),	\
	  /*LO*/exo_new(sn,ec_integer, (exo_integer_t)(REGS)->regs_C.lo),	\
	  /*FCC*/exo_new(sn,ec_integer, (exo_integer_t)(REGS)->regs_C.fcc),\
	  NULL)

#define MD_IREG_TO_EXO(REGS, IDX)					\
  exo_new(sn,ec_address, (exo_integer_t)(REGS)->regs_R[IDX])

#define MD_FREG_TO_EXO(REGS, IDX)					\
  exo_new(sn,ec_address, (exo_integer_t)(REGS)->regs_F.l[IDX])
...
#define MD_EXO_TO_MISC_REGS(EXO, ICNT, REGS)				\
  ...
  if (!exo								\
    ...
    fatal(sn,"could not read EIO misc regs");				\
...
void md_print_creg(md_ctrl_t regs, int reg, FILE *stream, int sn);
void md_print_cregs(md_ctrl_t regs, FILE *stream, int sn);
...
extern SS_ADDR_TYPE ss_lr_temp[MAX_SLOTS];
