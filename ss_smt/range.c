/*
 * modified range.c -  program execution range routines for SS_SMT 1.0 simulator
 *
 * This file was adapted by Ronaldo A. L. Goncalves using 
 * the original range.c file from SimpleScalar 3.0 Tool.
 *
 ...
 *
 * The original range.c file is a part of the SimpleScalar tool suite written by
 ...
 * The original range.c was adapted to support the ss_smt.c version 1.0.
 ...
 *  
 * $Id: range.c,v 1.2 1998/08/27 15:50:15 taustin Exp taustin $
 *
 * $Log: range.c,v $
                ...
                int sn)                         /* slot number */
{
  ...
  sym = sym_bind_name(s, NULL, sdb_any,sn);
  if (sym != NULL)
    {
                ...
                int sn)
{
  switch (pos->ptype)
    {
    case pt_addr:
      myfprintf(sn,stream, "@0x%08p", (md_addr_t)pos->pos);
      break;
    case pt_inst:
    ...
    case pt_cycle:
      ...
      break;
    default:
      panic(sn,"bogus execution position type");
    }
}
                  ...
                  int sn)                       /* slot number */
{
  ...
  *p = '\0';
  ...
  if (*pos1 && *pos1 != ':')
    {
      errstr = range_parse_pos(pos1, &range->start,sn);
      if (errstr)
	return errstr;
    }
  else
  ...
  if (*pos2)
	...
	{
	  errstr = range_parse_pos(pos2, &range->end,sn);
	  if (errstr)
	    return errstr;
	}
    }
...
range_print_range(struct range_range_t *range,	/* execution range */
		  FILE *stream,			/* output stream */
                  int sn)                       /* slot number */
{
  range_print_pos(&range->start, stream,sn);
  fprintf(stream, ":");
  range_print_pos(&range->end, stream,sn);
}
