/*
 * modified memory.c - flat memory space routines for SS_SMT 1.0 simulator
 *
 * This file was adapted by Ronaldo A. L. Goncalves using 
 * the original memory.c file from SimpleScalar 3.0 Tool.
 *
 ...
 *
 * The original memory.c file is a part of the SimpleScalar tool suite written by
 ...
 * The original memory.c was adapted to support the ss_smt.c version 1.0.
	      ...
	      struct stat_sdb_t *sdb)	/* stats data base */
{
  ...
  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.page_count_%s", mem[sn]->name, itoa[sn]);
     stat_reg_counter(sdb, buf, "total number of pages allocated",
		   &mem[sn]->page_count, mem[sn]->page_count, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.page_mem_%s", mem[sn]->name, itoa[sn]);
     sprintf(buf1, "%s.page_count_%s * %d / 1024", mem[sn]->name, itoa[sn], MD_PAGE_SIZE);
  stat_reg_formula(sdb, buf, "total size of memory pages allocated",
		   buf1, "%11.0fk",sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.ptab_misses_%s", mem[sn]->name, itoa[sn]);
     stat_reg_counter(sdb, buf, "total first level page table misses",
		   &mem[sn]->ptab_misses, mem[sn]->ptab_misses, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
     sprintf(buf, "%s.ptab_accesses_%s", mem[sn]->name, itoa[sn]);
     stat_reg_counter(sdb, buf, "total page table accesses",
		   &mem[sn]->ptab_accesses, mem[sn]->ptab_accesses, NULL,sn);
  }

  for (sn=0;sn<process_num;sn++) {
    sprintf(buf, "%s.ptab_miss_rate_%s", mem[sn]->name, itoa[sn]);
    sprintf(buf1, "%s.ptab_misses_%s / %s.ptab_accesses_%s", mem[sn]->name, itoa[sn], mem[sn]->name, itoa[sn]);
    stat_reg_formula(sdb, buf, "first level page table miss rate", buf1, NULL,sn);
  }
}
         ...
         int sn)                        /* slot number */
{
  ...
  while (len-- > 0)
    {
      fault = mem_access(mem, Read, addr, &data, sizeof(word_t),sn);
      if (fault != md_fault_none)
	return fault;

      myfprintf(sn,stream, "0x%08p: %08x\n", addr, data);
      addr += sizeof(word_t);
    }
           ...
           int sn)                      /* slot number */
{
  ...
  switch (cmd)
    ...
    case Read:
      ...
      do {
	fault = mem_fn(mem, Read, addr++, &c, 1,sn);
	if (fault != md_fault_none)
	  return fault;
	*s++ = c;
	n++;
    ...
    case Write:
      ...
      do {
	c = *s++;
	fault = mem_fn(mem, Write, addr++, &c, 1,sn);
	if (fault != md_fault_none)
	  return fault;
	n++;
      } while (c);
          ...
          int sn)
{
  ...
  while (nbytes-- > 0)
    {
      fault = mem_fn(mem, cmd, addr++, p++, 1,sn);
      if (fault != md_fault_none)
	return fault;
    }
           ...
           int sn)                      /* slot number */
{
  ...
  while (words-- > 0)
    {
      fault = mem_fn(mem, cmd, addr, p, sizeof(word_t),sn);
      if (fault != md_fault_none)
	return fault;
...
#define MEM_BLOCK_SIZE		0x10000
  ...
  if (!(/* text access and a read */
	(addr >= ld_text_base[sn] && addr < (ld_text_base[sn]+ld_text_size[sn])
	 && cmd == Read)
	/* data access within bounds */
	|| (addr >= ld_data_base[sn] && addr < ld_stack_base[sn])))
    fatal(sn,"access error: segmentation violation, addr 0x%08p", addr);
...
mem_init1(void)
{
  ...
  mem_brk_point = ROUND_UP(ld_data_base[sn] + ld_data_size[sn], SS_PAGE_SIZE);
