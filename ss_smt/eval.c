/*
 * modified expr.c -expression evaluator routines for SS_SMT 1.0 simulator
 *
 * This file was adapted by Ronaldo A. L. Goncalves using 
 * the original expr.c file from SimpleScalar 3.0 Tool.
 *
 ...
 * The original expr.c file is a part of the SimpleScalar tool suite written by
 ...
 * The original expr.c was adapted to support the ss_smt.c version 1.0.
 ...
 *
 * $Id: eval.c,v 1.3 1998/08/31 17:08:31 taustin Exp taustin $
 *
 * $Log: eval.c,v $
 * Revision 1.3  1998/08/31 17:08:31  taustin
 * fixed MS VC++ quad to double bugs in Alpha target
 *
...
enum eval_err_t eval_error[MAX_SLOTS];
...
static enum eval_token_t tok_map[MAX_SLOTS][256];

/* See eval_init for inicialization */
static int tok_map_initialized[MAX_SLOTS];


/* builds the first token map */
static void
init_tok_map(int sn) /* slot number */
{
  int i;

  for (i=0; i<256; i++)
    tok_map[sn][i] = tok_invalid;

  /* identifier characters */
  for (i='a'; i<='z'; i++)
    tok_map[sn][i] = tok_ident;
  for (i='A'; i<='Z'; i++)
    tok_map[sn][i] = tok_ident;
  tok_map[sn][(int)'_'] = tok_ident;
  tok_map[sn][(int)'$'] = tok_ident;

  /* numeric characters */
  for (i='0'; i<='9'; i++)
    tok_map[sn][i] = tok_const;
  tok_map[sn][(int)'.'] = tok_const;

  /* operator characters */
  tok_map[sn][(int)'+'] = tok_plus;
  tok_map[sn][(int)'-'] = tok_minus;
  tok_map[sn][(int)'*'] = tok_mult;
  tok_map[sn][(int)'/'] = tok_div;
  tok_map[sn][(int)'('] = tok_oparen;
  tok_map[sn][(int)')'] = tok_cparen;

  /* whitespace characers */
  tok_map[sn][(int)' '] = tok_whitespace;
  tok_map[sn][(int)'\t'] = tok_whitespace;
}
               ...
               int sn) /* slot number */
{
  ...
  if (!tok_map_initialized[sn])
    {
      init_tok_map(sn);
      tok_map_initialized[sn] = TRUE;
    }
  ...
  *ptok_buf = '\0';
  ...
  /* skip whitespace */
  while (*es->p && tok_map[sn][(int)*es->p] == tok_whitespace)
    es->p++;
  ...
  *ptok_buf++ = *es->p;
  tok = tok_map[sn][(int)*es->p++];
  switch (tok)
    {
    case tok_ident:
      /* parse off next identifier */
      while (*es->p
	     && (tok_map[sn][(int)*es->p] == tok_ident
		 || tok_map[sn][(int)*es->p] == tok_const))
	{
	  *ptok_buf++ = *es->p++;
	}
      break;
                ...
                int sn)                  /* slot number */
{
  ...
  if (es->peek_tok == tok_invalid)
    {
      es->lastp = es->p;
      es->peek_tok = get_next_token(es,sn);
    }
...
/* forward declaration */
static struct eval_value_t expr(struct eval_state_t *es, int sn);

/* default expression error value, eval_err is also set. It is a constant and does not need be replicated */
            ...
            int sn)                     /* slot number */
{
  ...
  if (t1 == et_symbol || t2 == et_symbol)
    panic(sn,"symbol used in expression");
...
double
eval_as_double(struct eval_value_t val, int sn)
{
  switch (val.type)
    {
    case et_double:
    ...
    case et_symbol:
      panic(sn,"symbol used in expression");
    default:
      panic(sn,"illegal arithmetic expression conversion");
    }
}

/* eval_value_t (any numeric type) -> float */
float
eval_as_float(struct eval_value_t val, int sn)
{
  switch (val.type)
    {
    case et_double:
...
quad_T
eval_as_quad(struct eval_value_t val, int sn)
{
  switch (val.type)
    {
    case et_double:
...
squad_T
eval_as_squad(struct eval_value_t val, int sn)
{
  switch (val.type)
    {
    case et_double:
...
md_addr_t
eval_as_addr(struct eval_value_t val, int sn)
{
  switch (val.type)
    {
    case et_double:
...
/* eval_value_t (any numeric type) -> unsigned int */
unsigned int
eval_as_uint(struct eval_value_t val, int sn)
{
  switch (val.type)
    {
    case et_double:
...
int
eval_as_int(struct eval_value_t val, int sn)
{
  switch (val.type)
    {
    case et_double:
...
f_add(struct eval_value_t val1, struct eval_value_t val2, int sn)
{
  enum eval_type_t et;
  ...
  et = result_type(val1.type, val2.type,sn);
  switch (et)
    {
    case et_double:
      val.type = et_double;
      val.value.as_double = eval_as_double(val1,sn) + eval_as_double(val2,sn);
      break;
    case et_float:
      val.type = et_float;
      val.value.as_float = eval_as_float(val1,sn) + eval_as_float(val2,sn);
      break;
#ifdef HOST_HAS_QUAD
    case et_quad:
      val.type = et_quad;
      val.value.as_quad = eval_as_quad(val1,sn) + eval_as_quad(val2,sn);
      break;
    case et_squad:
      val.type = et_squad;
      val.value.as_squad = eval_as_squad(val1,sn) + eval_as_squad(val2,sn);
      break;
#endif /* HOST_HAS_QUAD */
    case et_addr:
      val.type = et_addr;
      val.value.as_addr = eval_as_addr(val1,sn) + eval_as_addr(val2,sn);
      break;
    case et_uint:
      val.type = et_uint;
      val.value.as_uint = eval_as_uint(val1,sn) + eval_as_uint(val2,sn);
      break;
    case et_int:
      val.type = et_int;
      val.value.as_int = eval_as_int(val1,sn) + eval_as_int(val2,sn);
      break;
    default:
      panic(sn,"bogus expression type");
    }
...
/* compute <val1> - <val2> */
static struct eval_value_t
f_sub(struct eval_value_t val1, struct eval_value_t val2, int sn)
{
  enum eval_type_t et;
  struct eval_value_t val;
...
/* compute <val1> * <val2> */
static struct eval_value_t
f_mult(struct eval_value_t val1, struct eval_value_t val2,int sn)
{
  enum eval_type_t et;
  struct eval_value_t val;
...
/* compute <val1> / <val2> */
static struct eval_value_t
f_div(struct eval_value_t val1, struct eval_value_t val2, int sn)
{
  enum eval_type_t et;
  struct eval_value_t val;
...
/* compute - <val1> */
static struct eval_value_t
f_neg(struct eval_value_t val1, int sn)
{
  struct eval_value_t val;
...
/* compute val1 == 0 */
static int
f_eq_zero(struct eval_value_t val1, int sn)
{
  int val;
         ...
         int sn)  /* slot number */
{
  ...
  errno = 0;
  squad_val = myatosq(es->tok_buf, &endp, /* parse base */0,sn);
  if (!errno && !*endp)
    {
      /* good conversion */
      val.type = et_squad;
  ...
  errno = 0;
  quad_val = myatoq(es->tok_buf, &endp, /* parse base */0,sn);
  if (!errno && !*endp)
    {
      /* good conversion */
      val.type = et_quad;
       ...
       int sn)  /* slot number */
{
  ...
  tok = peek_next_token(es,sn);
  switch (tok)
    {
    case tok_oparen:
      (void)get_next_token(es,sn);
      val = expr(es,sn);
      if (eval_error[sn])
	return err_value;
     ...
     int sn)   /* slot number */
{
  ...
  switch (tok)
    ...
    case tok_div:
      ...
      if (eval_error[sn])
	return err_value;
      if (f_eq_zero(val1,sn))
	{
	  eval_error[sn] = ERR_DIV0;
	  return err_value;
	}
      val = f_div(val, val1,sn);
      break;
...
#ifdef HOST_HAS_QUAD
    case et_quad:
      myfprintf(sn,stream, "%lu [quad_T]", val.value.as_quad);
      break;
    case et_squad:
      myfprintf(sn,stream, "%ld [squad_T]", val.value.as_squad);
      break;
#endif /* HOST_HAS_QUAD */
    case et_addr:
      myfprintf(sn,stream, "0x%p [md_addr_t]", val.value.as_addr);
      break;
    case et_uint:
...
void eval_init_declarations(void){
     int sn;
  for (sn=0;sn<process_num;sn++) {
     tok_map_initialized[sn] = FALSE;
     eval_error[sn] = ERR_NOERR;
  }
}
...
struct eval_value_t
my_eval_ident(struct eval_state_t *es, int sn)
{
...
main(void)
{
  ...
  es = eval_new(my_eval_ident, NULL,sn);
  ...
  while (1)
	...
	exit(0);
      ...
      val = eval_expr(es, expr_buf, NULL,sn);
      if (eval_error[sn])
	fprintf(stdout, "eval error: %s\n", eval_err_str[eval_error[sn]]);
      else
	{
	  fprintf(stdout, "%s == ", expr_buf);
	  eval_print(stdout, val,sn);
	  fprintf(stdout, "\n");
	}
    }
}
