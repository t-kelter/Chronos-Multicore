/*
 * modified main.c - main line routines for SS_SMT 1.0 simulator
 *
 * This file was adapted by Ronaldo A. L. Goncalves using
 * the original main.c file from SimpleScalar 3.0 Tool.
 *
 ...
 * The original main.c file is a part of the SimpleScalar tool suite written by
 ...
 * The original main.c was adapted to support the ss_smt.c version 1.0.
 ...
 * 4) The return point was transfered to multiproc.c in order to allow 
 *    equivalent measurement among the slots
 * 5) A code block to interprete the USED_SLOT number on the command line
 *    was included int main() routine
 *
...
/* execution instruction counter. See sim_init for declaration initialization */
counter_t sim_num_insn[MAX_SLOTS];

/* these variables defines the first and last command line to be executed */

int start_command_line,
    last_command_line;
...
/* options database. The options are acessed by name, so  it is not necessary replicate this database. It is just necessary to create different name for a same option from different slot */
struct opt_odb_t *sim_odb;

/* stats database. The stats are acessed by name, so  it is not necessary replicate this database. It will be just necessary to create different name for a same stat from different slot */
struct stat_sdb_t *sim_sdb;

/* EIO interfaces. See sim_init for initializations */
char *sim_chkpt_fname = NULL;
char *sim_eio_fname[MAX_SLOTS] = {NULL};
FILE *sim_eio_fd[MAX_SLOTS] = {NULL};

/* redirected program/simulator output file names. To make easy, the output from simulator is just one for all slots */
static char *sim_simout = NULL;
static char *sim_progout = NULL;
FILE *sim_progfd = NULL;

/* redirected program/simulator input file names */
char *sim_inputname[MAX_SLOTS];
FILE *sim_inputfd[MAX_SLOTS];
...
{
  fprintf(fd, "Usage: %s <x1> <x2> {-options} <batch file>\n\n", argv[0]);

  fprintf(fd, "<x1> : initial line to consider from <batch file> (ignoring blank and comentaries lines)\n");
  fprintf(fd, "<x2> : final line to consider from <batch file> (ignoring blank and comentaries lines)\n");
  fprintf(fd, "{-options} : original options from sim-outorder plus some new ones\n");

  fprintf(fd, "<batch file> : text file: each line must be\n");
  fprintf(fd, "               a command line for one application that will\n");
  fprintf(fd, "               be executed for each slot\n\n");
...
main(int argc, char **argv, char **envp)
{
  ...
  /* to initialize vectorial variables */
  for (sn=0;sn<MAX_SLOTS;sn++)
  {  sim_inputname[sn] = NULL;
     sim_inputfd[sn] = NULL;
  }
  ...
  fatal_hook(sim_print_stats);   /* placed in misc.h/.c */
...
*/
  ...
  /* in this point they are necessary 4 arguments at least */

  if (argc < 4) {
     nArg = 1;
  }
  else  /* here it is supposed the second and third arguments are */
        /* the start command line and the finish command line inside */
        /* of a file with many command lines. The name of this file must */
        /* be provided as argument too. The value of process_num is got */
        /* from difference between the start and finish lines. */
  {
    start_command_line = atoi(argv[1]);
    last_command_line = atoi(argv[2]);
    process_num = last_command_line - start_command_line + 1;

    if ( !start_command_line ||
         !last_command_line ||
         (process_num < 1) ||
         (process_num > MAX_SLOTS) )
       fatal(0,"bad specification for first (=%s) and second (=%s) arguments", argv[1], argv[2]);
  ...
  /* To initialize all global variables from all modules, because after the replication they din't can be initialized on their declaration.*/

  sim_init_declarations();

  /* register global options */
  sim_odb = opt_new(orphan_fn,0);
  opt_reg_flag(sim_odb, "-h", "print help message",
	       &help_me, /* default */FALSE, /* !print */FALSE, NULL,0);
  opt_reg_flag(sim_odb, "-v", "verbose operation",
	       &verbose, /* default */FALSE, /* !print */FALSE, NULL,0);
#ifdef DEBUG
  opt_reg_flag(sim_odb, "-d", "enable debug message",
	       &debugging, /* default */FALSE, /* !print */FALSE, NULL,0);
#endif /* DEBUG */


  opt_reg_flag(sim_odb, "-Dinit", "start in Dlite debugger",
	       &dlite_initial, /* default */FALSE, /* !print */FALSE, NULL,sn);

  opt_reg_int(sim_odb, "-seed",
	      "random number generator seed (0 for timer seed)",
	      &rand_seed, /* default */1, /* print */TRUE, NULL,0);
  opt_reg_flag(sim_odb, "-q", "initialize and terminate immediately",
	       &init_quit, /* default */FALSE, /* !print */FALSE, NULL,0);
  opt_reg_string(sim_odb, "-chkpt", "restore EIO trace execution from <fname>", &sim_chkpt_fname, /* default */NULL, /* !print */FALSE, NULL,0);


  /* stdio redirection options */

  opt_reg_string(sim_odb,"-redir:sim",
        "redirect simulator output to file (non-interactive only)", &sim_simout, /* default */NULL, /* !print */FALSE, NULL,0);

  opt_reg_string(sim_odb,"-redir:prog",
    "redirect simulated program output to file",
	&sim_progout, /* default */NULL, /* !print */FALSE, NULL,sn);
  ...
  opt_reg_int(sim_odb, "-nice",
	      ...
	      /* default */NICE_DEFAULT_VALUE, /* print */TRUE, NULL,0);
#endif
...
/* FIXME: add stats intervals and max insts... */
  ...
  exec_index = -1;
  opt_process_options(sim_odb, argc, argv,0);
  ...
  if (sim_simout != NULL) {
	 ...
	 fatal(0,"unable to redirect simulator output to file `%s'", sim_simout);
   }
   ...
   if (sim_progout != NULL)
   {
      ...
      if (!sim_progfd)
	 fatal(0,"unable to redirect program output to file `%s'", sim_progout);
   }
...
nice_priority = nice(0);

/*  if (nice(0) != nice_priority)
    {
      if (nice(nice_priority - nice(0)) < 0)
	fatal(0,"could not renice simulator process");
    } */
#endif
  ...
  fprintf(stderr,"%s %d %d ", argv[0], start_command_line, last_command_line);
  ...
  md_init_decoder(); /* it is placed in the "machine.h/.c" */
