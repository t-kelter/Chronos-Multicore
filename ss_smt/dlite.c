 ...
 * the original dlite.c file from SimpleScalar 3.0 Tool.
 *
 ...
 *
 * The original dlite.c file is a part of the SimpleScalar tool suite written by
 ...
 * The original dlite.c was adapted to support the ss_smt.c version 1.0.
 ...
 *
 * $Id: dlite.c,v 1.2 1998/08/27 08:12:26 taustin Exp $
 *
 * $Log: dlite.c,v $
...
/* set non-zero to enter DLite after next instruction. See dlite_init for the initializatrions  */
int dlite_active[MAX_SLOTS];
int dlite_initial;

/* non-zero to force a check for a break. See dlite_init for the initializatrions  */
int dlite_check[MAX_SLOTS];

/* set non-zero to exit DLite command loop. See dlite_init for the inicializatrions  */
static int dlite_return[MAX_SLOTS];
...
#define MOD_QUAD	0x0008		/* q - print a quadword */
...
#define MOD_SIZES							\
  (MOD_BYTE|MOD_HALF|MOD_WORD|MOD_QUAD					\
...
/* DLite default expression evaluator. See in the dlite_init for the inicializations */
static struct eval_state_t *dlite_evaluator[MAX_SLOTS];
static struct regs_t *local_regs[MAX_SLOTS];
                ...
                int sn) /* slot number */
{
 ...
 /* the variable "err_value" doesnot need be "static" because it is never modified */
 /* static */ struct eval_value_t err_value = { et_int, { 0 } };
  ...
  for (i=0; md_reg_names[i].str != NULL; i++)
	...
	{
	  err_str =
	    f_dlite_reg_obj(local_regs[sn], /* !is_write */FALSE,
			    md_reg_names[i].file, md_reg_names[i].reg, &val,sn);
	  if (err_str)
	    {
	      eval_error[sn] = ERR_UNDEFVAR;
	      val = err_value;
	    }
	  return val;
	}
  ...
  sym_loadsyms(ld_prog_fname[sn], /* load locals */TRUE,sn);
  sym = sym_bind_name(es->tok_buf, NULL, sdb_any,sn);
  if (sym)
    {
  ...
  if (stat)
	...
	case sc_quad:
	  val.type = et_quad;
	  val.value.as_quad = *stat->variant.for_quad.var;
	  break;
#endif /* HOST_HAS_QUAD */
	case sc_float:
	  val.type = et_float;
	...
	case sc_dist:
	case sc_sdist:
	  eval_error[sn] = ERR_BADEXPR;
	  val = err_value;
	  break;
	case sc_formula:
	  {
	    /* instantiate a new evaluator to avoid recursion problems */
	    struct eval_state_t *es = eval_new(ident_evaluator, sim_sdb,sn);
	    char *endp;
...
dlite_help(int nargs, union arg_val_t args[],
	   struct regs_t *regs, struct mem_t *mem, int sn);
static char *
           ...
           int sn)                              /* slot number */
{
  ...
  *q = '\0';
  ...
  for (i=0, arg_cnt=0; i<MAX_ARGS && cmd->arg_strs[i] != NULL; i++, arg_cnt++)
	...
	case 'a':
	  local_regs[sn] = regs;
	  val = eval_expr(dlite_evaluator[sn], p, &endp,sn);
	  if (eval_error[sn])
	    return eval_err_str[eval_error[sn]];
	...
	case 's':
	  ...
	  *q = '\0';
	  ...
	  break;
	default:
	  panic(sn,"bogus argument type: `%c'", arg_type);
	}
      p = endp;
    }
  ...
  return cmd->cmd_fn(arg_cnt, args, regs, mem,sn);
}
          ...
          int sn)                               /* slot number */
{
  ...
  if ((modifiers & MOD_SIZES) == 0)
	...
	case et_addr:
	  ...
	  break;
#ifdef HOST_HAS_QUAD
	case et_quad:	modifiers |= MOD_QUAD; break;
	case et_squad:	modifiers |= MOD_QUAD; break;
#endif /* HOST_HAS_QUAD */
...
#ifdef HOST_HAS_QUAD
	case et_quad:	modifiers |= MOD_UNSIGNED; break;
	case et_squad:	modifiers |= MOD_DECIMAL; break;
#endif /* HOST_HAS_QUAD */
  ...
  /* decode modifiers */
  if (modifiers & (MOD_BYTE|MOD_HALF|MOD_WORD|MOD_QUAD))
    {
	...
	panic(sn,"no default integer format");
      ...
      if (modifiers & MOD_BYTE)
	{
	  ...
	  myfprintf(sn,stdout, buf, eval_as_uint(val,sn));
	}
      ...
      else if (modifiers & MOD_QUAD)
	{
	  ...
	  myfprintf(sn,stdout, buf, eval_as_quad(val,sn));
	}
#endif /* HOST_HAS_QUAD */
    }
  else if (modifiers & MOD_FLOAT)
    fprintf(stdout, "%f", (double)eval_as_float(val,sn));
  else if (modifiers & MOD_DOUBLE)
    fprintf(stdout, "%f", eval_as_double(val,sn));
  else if (modifiers & MOD_CHAR)
    fprintf(stdout, "`%c'", eval_as_uint(val,sn));
...
static void
print_help(struct dlite_cmd_t *cmd, int sn)		/* command to describe */
{
  int i;
           ...
           int sn) /* slot number */
{
  ...
  if (!f_dlite_reg_obj || !f_dlite_mem_obj)
    panic(sn,"DLite is not configured");
  ...
  if (nargs == 1)
	...
	return "address argument must be an integral type";
      ...
      /* reset PC */
      val.type = et_addr;
      val.value.as_addr = eval_as_addr(args[0].as_value, sn);
      f_dlite_reg_obj(regs, /* is_write */TRUE, rt_PC, 0, &val,sn);

      myfprintf(sn,stdout, "DLite: continuing execution @ 0x%08p...\n",
		val.value.as_addr);
    }

  /* signal end of main debugger loop, and continuation of prog execution */
  dlite_active[sn] = FALSE;
  dlite_return[sn] = TRUE;
            ...
            int sn)                             /* slor number */
{
  ...
  err_str = print_val(modifiers, val,sn);
  if (err_str)
    return err_str;
  fprintf(stdout, "\n");
             ...
             int sn)                            /* slot number */
{
  ...
  opt_print_option(opt, stdout,sn);
  if (opt->desc)
           ...
           int sn)                              /* slot number */
{
  ...
  /* else, print this option's value */
  stat_print_stat(sim_sdb, stat, stdout,sn);
            ...
            int sn)                             /* slot number */
{
  ...
  myfprintf(sn,stdout, "PC: 0x%08p   NPC: 0x%08p\n",
	    regs->regs_PC, regs->regs_NPC);
  if ((err_str = dlite_cregs(nargs, args, regs, mem,sn)) != NULL)
    return err_str;
...
{
  /* print floating point registers */
  md_print_cregs(regs->regs_C, stdout,sn);
           ...
           int sn)                              /* slot number */
{
  ...
  md_print_iregs(regs->regs_R, stdout);
  dlite_pause();
  if ((err_str = dlite_fpregs(nargs, args, regs, mem, sn)) != NULL)
    return err_str;
  ...
  if (f_dlite_mstate_obj)
    {
      if (nargs == 0)
	{
	  errstr = f_dlite_mstate_obj(stdout, NULL, regs, mem,sn);
	  if (errstr)
	    return errstr;
	}
      else
	{
	  errstr = f_dlite_mstate_obj(stdout, args[0].as_str, regs, mem,sn);
	  if (errstr)
	    return errstr;
	}
    }
              ...
              int sn)          /* slot number */
{
  ...
  if (nargs == 1)
	...
	return "address argument must be an integral type";
      ...
      /* reset address */
      addr = eval_as_addr(args[0].as_value,sn);
    }
  else if (nargs == 2)
    {
      modifiers = args[0].as_modifier;
	...
	return "address argument must be an integral type";
      ...
      /* reset address */
      addr = eval_as_addr(args[1].as_value,sn);
    }
  ...
  if (modifiers & (MOD_BYTE|MOD_CHAR))
    ...
    size = 2;
  else if (modifiers & (MOD_QUAD|MOD_DOUBLE))
    size = 8;
  else /* no modifiers, or MOD_WORD|MOD_FLOAT */
    size = 4;

  /* read memory */
  errstr = f_dlite_mem_obj(mem, /* !is_write */FALSE, addr, (char *)buf, size, sn);
  if (errstr)
    return errstr;
  ...
  /* print the value */
  errstr = print_val(modifiers, val,sn);
  if (errstr)
    return errstr;
  fprintf(stdout, "\n");
...
/* the variable "addr" is static, then it must be replicated, but its inicialization must happen just one time, during the first time that function is called for each slot */

   static md_addr_t addr[MAX_SLOTS]; 
   static int first_time = 1;

   if (first_time) {
     for (i=0;i<process_num;i++) addr[i] = 0;
     first_time = 0;
   }
  ...
  if (nargs == 1)
	...
	return "address argument must be an integral type";
      ...
      /* reset PC */
      addr[sn] = eval_as_addr(args[0].as_value,sn);
    }
  else if (nargs == 2)
    {
      /* check address */
  ...
  if (fmt_lines == 1)
    {
      /* unformatted dump */
      i_addr = fmt_addr;
      myfprintf(sn,stdout, "0x%08p: ", i_addr);
      for (i=0; i < count; i++)
	{
	  errstr =
	    f_dlite_mem_obj(mem, /* !is_write */FALSE,
			    i_addr, (char *)&byte, 1,sn);
	  if (errstr)
	    return errstr;
      ...
      for (i=0; i < fmt_lines; i++)
	{
	  ...
	  for (j=0; j < BYTES_PER_LINE; j++)
	    {
	      if (i_addr >= addr[sn] && i_count <= count)
		{
		  errstr =
		    f_dlite_mem_obj(mem, /* !is_write */FALSE,
				    i_addr, (char *)&byte, 1,sn);
  ...
  for (i=0; i<count; i++)
    {
      ...
      myfprintf(sn,stdout, "    0x%08p:   ", addr[sn]);
      errstr =
	f_dlite_mem_obj(mem, /* !is_write */FALSE,
			addr[sn], (char *)&inst, sizeof(inst),sn);
      if (errstr)
	return errstr;
      md_print_insn(inst, addr[sn], stdout);
      fprintf(stdout, "\n");

      /* go to next instruction */
      addr[sn] += sizeof(md_inst_t);
    }
...
/* all active break points, in a list. See dlite_init for the inicialization */
static struct dlite_break_t *dlite_bps[MAX_SLOTS];

/* unique id of next breakpoint. See dlite_init for inicialization */
static int break_id[MAX_SLOTS];

/* return breakpoint class as a string */
static char *					/* breakpoint class string */
bp_class_str(int class, int sn)			/* breakpoint class mask */
{
          ...
          int sn)   /* slot number */
{
  ...
  bp->next = dlite_bps[sn];
  dlite_bps[sn] = bp;

  fprintf(stdout, "breakpoint #%d set @ ", bp->id);
  range_print_range(&bp->range, stdout,sn);
  fprintf(stdout, ", class: %s\n", bp_class_str(class,sn));
...
static char *					/* err str, NULL for no err */
delete_break(int id, int sn)				/* id of brkpnt to delete */
{
  ...
  for (bp=dlite_bps[sn],prev=NULL; bp != NULL; prev=bp,bp=bp->next)
    {
      if (bp->id == id)
	break;
    }
...
/* this variable clues dlite_main() into why it was called. See for initialization */
static int break_access[MAX_SLOTS];
              ...
              int sn)    /* slot number */
{
  ...
  for (bp=dlite_bps[sn]; bp != NULL; bp=bp->next)
	  ...
	  if ((bp->class & ACCESS_WRITE)
		  ...
		  && !range_cmp_range(&bp->range, addr,sn)))
	    {
	      ...
	      myfprintf(sn,stdout,
			...
			bp->id, addr);
	      break_access[sn] = ACCESS_WRITE;
	      return TRUE;
	    }
	  break;

	case pt_inst:
	  if (!range_cmp_range(&bp->range, icount,sn))
	    {
	      /* hit a code breakpoint */
	      fprintf(stdout,
	...
	case pt_cycle:
	  if (!range_cmp_range(&bp->range, cycle,sn))
	    {
	      /* hit a code breakpoint */
	      fprintf(stdout,
            ...
            int sn)                             /* slot number */
{
  ...
  return set_break(ACCESS_EXEC, &range,sn);
}
             ...
             int sn)                            /* slot number */
{
  ...
  /* set the breakpoint */
  return set_break(access, &range,sn);
}
             ...
             int sn)                            /* slot number */
{
  ...
  /* check range */
  errstr = range_parse_range(args[0].as_str, &range,sn);
  if (errstr)
    return errstr;
             ...
             int sn)                            /* slot number */
{
  ...
  id = eval_as_uint(args[0].as_value,sn);
  return delete_break(id,sn);
}
            ...
            int sn) /* slot number */
{
  ...
  while (dlite_bps[sn] != NULL)
    {
      /* delete first breakpoint */
      delete_break(dlite_bps[sn]->id,sn);
    }
              ...
              int sn)                           /* slot number */
{
  ...
  sym_loadsyms(ld_prog_fname[sn], /* !locals */FALSE,sn);

  /* print all symbol values */
  for (i=0; i<sym_nsyms[sn]; i++)
    sym_dumpsym(sym_syms[sn][i], stdout);
               ...
               int sn)    /* slot number */
{
  ...
  for (i=0; i<sym_ntextsyms[sn]; i++)
    sym_dumpsym(sym_textsyms[sn][i], stdout);
               ...
               int sn)    /* slot number */
{
  ...
  for (i=0; i<sym_ndatasyms[sn]; i++)
    sym_dumpsym(sym_datasyms[sn][i], stdout);
             ...
             int sn)                            /* slot number */
{
  ...
  /* print a single option, specified by argument */
  sym = sym_bind_name(args[0].as_str, &index, sdb_any,sn);
  if (!sym)
    return "symbol is not defined";

  /* else, print this symbols's value */
  sym_dumpsym(sym_syms_by_name[sn][index], stdout);
...
dlite_init(dlite_reg_obj_t reg_obj,		/* register state object */
   ...
   for (sn=0; sn<process_num; sn++) {
      /* Here there are some inicializations */

      dlite_active[sn] = FALSE;
      dlite_check[sn] = FALSE;
      dlite_return[sn] = FALSE;
      /*    dlite_evaluator[sn] = NULL; */
      local_regs[sn] = NULL;
      ...
      dlite_evaluator[sn] = eval_new(ident_evaluator, NULL,sn);
   }
             ...
             int sn)   /* slot number */
{
  ...
  if (dbreak)
    {
      ...
      myfprintf(sn,stdout, "[%10n] 0x%08p:    ", cycle, regs_PC);
      errstr =
	f_dlite_mem_obj(mem, /* !is_write */FALSE,
			regs_PC, (char *)&inst, sizeof(inst),sn);
      if (errstr)
  ...
  /* read and disassemble instruction */
  myfprintf(sn,stdout, "[%10n] 0x%08p:    ", cycle, next_PC);
  errstr =
    f_dlite_mem_obj(mem, /* !is_write */FALSE,
		    next_PC, (char *)&inst, sizeof(inst),sn);
  if (errstr)
    fprintf(stdout, "<invalid memory>: %s", errstr);
  else
           ...
           int sn)                              /* slot number */
{
  ...
  int i;
  int dbreak = (break_access[sn] & (ACCESS_READ|ACCESS_WRITE)) != 0;

  /* the variable "cmd" is static, then it must be replicated, but its inicialization must happen just one time, during the first time that function is called for each slot */

  static char cmd[MAX_SLOTS][512];
  static int first_time = 1;

  if (first_time) {
     for (i=0;i<process_num;i++) strcpy(cmd[i],"");
     first_time = 0;
  }

  /* so, it is necessary creates one auxiliar variable to contain the information if */

  dlite_active[sn] = TRUE;
  dlite_return[sn] = FALSE;
  dlite_status(regs_PC, next_PC, cycle, dbreak, regs, mem,sn);

  while (dlite_active[sn] && !dlite_return[sn])
    {
      ...
      if (buf[0] != '\0')
	{
      ...
      err_str = dlite_exec(cmd[sn], regs, mem,sn);
      if (err_str)
