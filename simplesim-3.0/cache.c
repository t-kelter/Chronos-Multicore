...
/* sudiptac :::: for accounting shared bus */
extern sched_p global_sched_data;
extern ull total_bus_delay;
/* effective code range */
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  int lat = 0;
  /* sudiptac :::: for accounting bus related delay */
  uint bus_delay = 0;
  ...
  /* sudiptac :::: for accounting bus delay */
  /* FIXME : Currently the core is always set to 
	* first one */
   bus_delay = compute_bus_delay((ull)now, 0);		  		  
	/* Add the bus delay only if the instruction access is in 
	 * effective code range. Otherwise it is in library code 
	 * and no need to encounter it in effective simulation cycle 
	 */
   if(addr >= start_addr && addr <= end_addr)
		total_bus_delay += bus_delay;

  /* sudiptac ::: for debugging cache miss accesses */

#ifdef _DEBUG
  extern FILE* ifp;
  if(!ifp)
	 ifp = fopen("cache_access.db", "w");
  if(!ifp)
  {
	  fprintf(stdout, "Internal file opening failed\n");  
	  exit(-1);
  }
  fprintf(ifp, "Cache access at = 0x%x at sim cycle %Lu\n", (unsigned)addr, (ull)now);
  fprintf(ifp, "****Bus delay encountered = %u****\n", bus_delay);
  fprintf(ifp, "****Total bus delay so far = %u****\n", total_bus_delay);
  if(addr < start_addr || addr > end_addr)
	fprintf(ifp,"Instruction access is in library code\n");
#endif
  ...
  if (repl->status & CACHE_BLK_VALID)
	...
	*repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
      ...
      /* by Chronos: we assuem non-blocking il1 cache when misprediction
       * happens, i.e., outstanding miss during misprediction does not postpone
       * instruction fetch along correct path after branch resolving */
      lat = 0;
  ...
  /* return lat; */
  /* sudiptac ::: accounting the bus delay */
  return (lat + bus_delay);
