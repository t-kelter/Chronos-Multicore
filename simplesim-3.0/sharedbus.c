...
static uint compute_waiting_time(core_sched_p head_core, ull start_time)
{
	long long delta, check_len;
	uint delay_elem;
	...
	{
		check_len = (delta - ((delta/head_core->interval) * head_core->interval)); 	 
		if(check_len < head_core->slot_len)	 
			delay_elem = 0;
		else
		{
 ...
 * calculates the variaable memory latency for the request */
uint compute_bus_delay(ull start_time, uint ncore)
{
	...
	add_delay = compute_waiting_time(cur_core_sched, start_time);
