...
/* Chronos notice:
 *------------------------------------------------------------------------------
 * We have made some changes to adjust the behavior of SimpleScalar
 * out-of-order pipeline to the simplified architecture modeled in Chronos. The
 * changes are annotated with comments containing "Chronos".
 *------------------------------------------------------------------------------
 */
...
extern char* tdma_sched_filename;
extern sched_p global_sched_data;
/* END */

// Variables defined by Chronos
// code in [start_addr, end_addr] are user code, and execution of library code which
// is // outside this scope will not be taken into account in our statistics
extern md_addr_t    start_addr, end_addr;
// effective execution cycles by excluding library execution
extern int	    effect_cycles;
// effective instruction cache misses by excluding library execution
...
/* 
 * Chronos: the configuraion of following functional units have been changed to
 * keep in consistence with the default configuration of Chronos
 */
...
/* Chronos: to count i-cache misses only in normal execution */
static int my_last_inst_missed = FALSE;

// by Chronos: dump branch predictor states
void
dump_bpred()
{
    int		i, size, shiftreg;
...
/* commented by Chronos: we do not support the extra misprediction penalties
...
struct RUU_station {
  ...
  int slip;
  /* instruction status */
  int icache_miss;			/* by Chronos: for i-cache stats */
...
static void
my_ruu_dumpent(struct RUU_station *rs,		/* ptr to RUU station */
	    ...
	    int header)				/* print header? */
{
  ...
  fprintf(stream, "%2d: %x: ", index, rs->PC);
  md_print_insn(rs->IR, rs->PC, stream);
  fprintf(stream, "'\n");
  fprintf(stream, "   ready: %s, queued: %s, issued: %s, completed: %s, ic_miss: %s\n",
...
/* dump the contents of the RUU */
static void
my_ruu_dump(FILE *stream)				/* output stream */
{
  int num, head;
  ...
  fprintf(stream, "RUU_head: %d, RUU_tail: %d, ", RUU_head, RUU_tail);
  ...
  while (num)
    {
      rs = &RUU[head];
      my_ruu_dumpent(rs, rs - RUU, stream, /* header */TRUE);
...
/* by Chronos: above scheduling policy is modified to oldest first */
static void
readyq_enqueue(struct RUU_station *rs)		/* RS to enqueue */
{
  ...
  /* locate insertion point */
  /* by Chronos: change the scheduling policy to oldest first */
#if 0
...
ruu_commit(void)
{
  ...
  while (RUU_num > 0 && committed < ruu_commit_width)
  {
      ...
      */
      ...
      {
	  // by Chronos: important change to original code:
...
ruu_dispatch(void)
{
  ...
  int icache_miss;			/* by Chronos: is it an i-cache miss */
  ...
  while (/* instruction decode B/W left? */
	      ...
	      && !OPERANDS_READY(last_op.rs)))
	{
      ...
      // by Chronos, SimpleScalar allows spec exec. in a pending spec exec, but
      // our estimation only allows one level of spec, so we need to stall the
      // pipeline from decoding and issuing exec. beyond a cond. branch inside
      // a spec
      if (spec_mode 
	  && !pred_perfect
	...
	break;
      ...
      icache_miss = fetch_data[fetch_head].icache_miss;
...
#include "machine.def"
	      ...
	      && target_PC != pred_PC && br_pred_taken))
	{
	  ...
	  // by Chronos: dont' let unconditional branches stall the fetcher
	  if (pred_perfect
	  ...
	  if (pred_PC != regs.regs_NPC && !fetch_redirected)
	    {
	      ...
	      /* Chronos: not counting cache misses in misprediction */
	      my_last_inst_missed = FALSE;
//fprintf(stderr, "reset my_last_inst_missed at cycle %d\n", sim_cycle);
	    }
	}
  ...
  for (i=0, branch_cnt=0;
	  ...
	  && !(fetch_regs_PC & (sizeof(md_inst_t)-1)))
	{
	  ...
	  if (cache_il1)
		...
		if (!spec_mode)
		    my_last_inst_missed = TRUE;
                md_addr_t addr; 
                addr = IACOMPRESS(fetch_regs_PC);
		// Chronos (26/06/2006/): icache miss stats are now moved to
		// commit stage to avoid counting cache misses during mispredictions
      ...
      if (pred)
	{
	  ...
	  if (MD_OP_FLAGS(op) & F_CTRL) {
	    // by Chronos: this is incorrect: to determine the BHT entry to be
      ...
      else
	{
      ...
      fetch_data[fetch_tail].icache_miss = my_last_inst_missed;
...
#include "machine.def"
  ...
  /* sudiptac :::: Set up global TDMA bus schedule */
  setSchedule(tdma_sched_filename);
