...
/* Chronos notice:
 *------------------------------------------------------------------------------
 * The default statistics are both much more than what we are concerned with and
 * the concerned ones do not meet our needs.
 * Thus we have added variables (e.g. effect_cycles for the statistics of effect
 * simulation cycles excluding the executionof library functions
 * preceding/succeeding the user code); and we have added code in exit_now() to
 * dump out the statistics that we are concerned with. 
 *------------------------------------------------------------------------------
 */
...
#include "sim.h"
#include "symbol.h"
/* sudiptac :::: for shared TDMA bus schedule */
#include "sharedbus.h"
...
#endif
/* sudiptac :::: for shared TDMA bus schedule */
sched_p global_sched_data = NULL;
ull total_bus_delay = 0;	
char* tdma_sched_filename = "TDMA_bus_sched.db";
/********** END TDMA BUS **********************/
int	    effect_cycles = 0;
int	    effect_bpred_count = 0;
int	    effect_bpred_misses = 0;
int       effect_icache_misses = 0;
md_addr_t   start_addr, end_addr;
int	    *inst_cnts;
...
exit_now(int exit_code)
{
  ...
  char icache_str[64];
  char busdelay_str[64];
  char simbuscycle_str[64];
   ...
   */
  sprintf(banner_str, "+--------------------------------------------------------------+");
  sprintf(addr_str, "%-30s[%x, %x]", "effective code range:", start_addr, end_addr - sizeof(md_inst_t));
  sprintf(bpred_count_str, "%-30s%d", "effective bpred count:", effect_bpred_count);
  sprintf(cycles_str, "%-30s%d", "effective sim cycles:", effect_cycles);
  sprintf(bpred_miss_str, "%-30s%d", "effective bpred misses:", effect_bpred_misses);
  sprintf(icache_str, "%-30s%d", "effective icache misses:", effect_icache_misses);
  /* sudiptac :::: for accounting the shared bus delay */
  sprintf(busdelay_str, "%-30s%d", "total bus access delay:", total_bus_delay);
  sprintf(simbuscycle_str, "%-30s%d", "sim cycles with shared bus:", effect_cycles + total_bus_delay);
