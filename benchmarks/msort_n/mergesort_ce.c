...
void main() {
    ...
    #if DEBUG
    fprintf(f,"Pass flag 1\n");fflush(f);
    #endif
    #if DEBUG
    fprintf(f,"final\n");fflush(f);
    for (i=0;i<SIZE;i++) fprintf(f,"%d ",arrayIn[i]);fprintf(f,"\n");
    #endif
    ...
    while (runsize<SIZE) {
        ...
        while(num*runsize<SIZE && kk<SIZE && jj< SIZE) {
            ...
            #if DEBUG
            fprintf(f,"Merge two arrays\n");
            for (h=low;h<mid;h++) fprintf(f,"%d ",arrayIn[h]);fprintf(f,"\n");
            for (h=mid;h<high;h++) fprintf(f,"%d ",arrayIn[h]);fprintf(f,"\n");
            #endif
        ...
        //for (i=0;i<SIZE;i++) arrayIn[i]=arrayOut[i];
            #if DEBUG
            fprintf(f,"Merged array\n");
            for (h=low;h<high;h++) fprintf(f,"%d ",arrayIn[h]);
            #endif
        runsize*=2;
    }
    #if DEBUG
    fprintf(f,"final\n");fflush(f);
    for (i=0;i<SIZE;i++) fprintf(f,"%d ",arrayIn[i]);
    #endif
}
